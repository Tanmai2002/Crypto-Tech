...
app.config['MYSQL_DB'] = 'CryptoWallet'
...
def register():
    ...
    if request.method == 'POST' and form.validate():
        ...
        if isnewuser(form.username.data):
...
def transaction():
    ...
    if request.method == 'POST':
        try:
            ...
            success = send_money(session.get('username'), form.username.data, form.amount.data)
            if success:
                flash("Money Sent!", "success")
            else:
                flash("Someone Modified Blockchain Brutally!", "danger")
...
def buy():
    ...
    if request.method == 'POST':
        ...
        try:
            success = send_money("BANK", session.get('username'), form.amount.data)
            if success:
